...
char *arithmatoy_mul(unsigned int base, const char *lhs, const char *rhs) {
  ...
  DynamicArray **results = calloc(lhs_len * rhs_len, sizeof(DynamicArray));
  int index = 0;
  for(int i=0; i < lhs_len; i++){
    for(int j=0; j < rhs_len; j++){
      DynamicArray *result = createDynamicArray();
      int calcul = get_digit_value(rev_lhs[i]) * get_digit_value(rev_rhs[j]) * (my_pow(base, i) == 0? 1:my_pow(base, i)) * (my_pow(base, j) == 0? 1:my_pow(base, j));
      int res;
      if(!calcul) append(result, '0');
      // printf("a[%d] * b[%d] = %c * %c = %s\n", i, j, rev_lhs[i], rev_rhs[j], result->data);

      while(calcul != 0){
        res = calcul % base;
        calcul /= base;
        append(result, to_digit(res));
      }
      append(result, '\0');
      reverse(result->data);
      // printf("a[%d] * b[%d] = %c * %c = %s\n", i, j, rev_lhs[i], rev_rhs[j], result->data);
  ...
  char *final_result = multiple_adding(results, lhs_len * rhs_len, base);

  char* end = calloc(strlen(final_result), sizeof(char));
  memcpy(end, final_result, strlen(final_result) * sizeof(char));

  for(int i=0; i < lhs_len * rhs_len; i++){
    destroyDynamicArray(results[i]);
  }
  free(results);
  free(rev_lhs);
  free(rev_rhs);
